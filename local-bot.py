...
def create_ai_strategies(model_name):
    ...
    if model_name == 'Genetic Algorithm':
        ...
        for _ in range(10):
            params = random.choice(common_strategies)
            params['target_profit'] += random.uniform(0, 0.5)
...
def main():
    ...
    strategy_count = 0
    while strategy_count < 70:  # Just for local testing, 70 strategies in total
        ...
        for strategy in new_strategies:
            ...
            # Maintain the top 20 strategies for local testing
            strategy_db = sorted(strategy_db, key=lambda x: x['result'], reverse=True)[:20]

        strategy_count += 10

    # Display top 20 strategies
    display_top_strategies(strategy_db)

def display_top_strategies(strategy_db):
    print("\nTop 20 Strategies (Sorted by Profitability):\n")
    for i, strategy in enumerate(strategy_db):
        print(f"Rank {i+1}:")
        print(f"AI Model: {strategy.get('model', 'Common')}")
        print(f"Parameters: {json.dumps(strategy, indent=4)}")
        print(f"Profitability: {strategy['result']}")
        print('-' * 40)

# Predefined common strategies
common_strategies = [
    ...
    {'macd_fast': 14, 'macd_slow': 30, 'macd_signal': 12, 'rsi_overbought': 72, 'rsi_oversold': 28, 'adx_threshold': 20, 'target_profit': 1.75, 'atr_multiplier': 2.2},
